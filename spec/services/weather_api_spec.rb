...
RSpec.describe WeatherApi do
  ...
  describe ".get_forecast" do
    context "when given a valid location" do
      ...
      it "queries Weather API for current forecast" do
        result = described_class.get_forecast("Kitchener, Ontario, Canada")
        expect(result).to eq({
          data: wx_forecast,
          cached: false
        })
      ...
      it "caches the result" do
        # First call to API (gets cached)
        result1 = described_class.get_forecast("Vancouver Canada")
        expect(result1[:cached]).to eq(false)

        # Subsequent call (retrieved from cache)
        result2 = described_class.get_forecast("Vancouver Canada")
        expect(result2[:cached]).to eq(true)

        expect(WebMock).to have_requested(:get, /forecast\.json/).once
      end
    end
  end
end
